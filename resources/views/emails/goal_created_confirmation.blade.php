...
<html lang="en">
    <head>
        ...
        <title>Goal Created Confirmation</title>
    </head>
    <body>
        <style>
            html,
                    ...
                    <td align="left" valign="center">
                        <div style="text-align: left; margin: 0 20px; padding: 40px; background-color: #ffffff; border-radius: 6px;">
                            ...
                            <div style="padding-bottom: 30px; font-size: 18px;">
                                <strong>🎯 Your Goal Has Been Created!</strong>
                            </div>
                            <div style="padding-bottom: 40px; font-family: Poppins, Helvetica, sans-serif; font-size: 14px;">
                                <p>Hi {{ $goal->user->first_name }},</p>
                                <p>Your goal "<strong>{{ $goal->title }}</strong>" has been successfully created.</p>
                                <p><strong>Goal Details:</strong></p>
                                <ul>
                                    <li>Target Amount: <strong>${{ number_format($goal->target_amount, 2) }}</strong></li>
                                    <li>Target Date: <strong>{{ Carbon::parse($goal->target_date)->toFormattedDateString() }}</strong></li>
                                    <li>Priority: <strong>{{ ucfirst($goal->priority) }}</strong></li>
                                </ul>
                                <p>Thank you for using {{ config('app.name') }} to stay financially focused!</p>
                                <p>
                                    <a href="{{ route('user.goal') }}" role="button" style="display: inline-block; padding: 10px 20px; background: #007bff; color: white; text-decoration: none; border-radius: 5px; font-size: 14px;">View My Goals</a>
                                </p>
                            </div>
