            ...
            <div class="col-12 col-sm">
                <nav aria-label="breadcrumb" class="mb-2">
                    <ol class="breadcrumb mb-0">
                        ...
                        <li class="breadcrumb-item active bi" aria-current="page">Make Wire Transfer</li>
                    </ol>
                </nav>
                <h5>Make Wire Transfer</h5>
            </div>
        </div>
                ...
                <div class="row">
                    ...
                    <div class="col-12 col-lg-12 mb-4">
                        <div class="card adminuiux-card h-100 rounded-4 border">
                            ...
                            <div class="card-body">
                                <form id="wire-transfer" action="{{ route('admin.wire.store') }}" method="POST">
                                    @csrf
                                    <div class="row">
                                        <div class="col-md-6">
                                            <div class="form-floating mb-4">
                                                <select class="form-select rounded-4 @error('sender') is-invalid @enderror" id="sender" name="sender">
                                                    <option value="" disabled {{ old('sender') ? '' : 'selected' }}>Select Sender</option>
                                                    @foreach($users as $user)
                                                        <option value="{{ $user->id }}" {{ old('sender') == $user->id ? 'selected' : '' }} data-name="{{ $user->first_name }} {{ $user->last_name }}">
                                    ...
                                    <div class="row mb-5">
                                        <div class="col">
                                            <div class="form-floating mb-4">
                                                <input type="text" class="form-control rounded-4 @error('account_number') is-invalid @enderror" name="account_number" id="account_number" value="{{ old('account_number') }}">
                                        ...
                                        <div class="col">
                                            <div class="form-floating mb-1">
                                                <input type="text" class="form-control rounded-4 @error('amount') is-invalid @enderror" name="amount" id="wire_transfer_amount" value="{{ old('amount') }}">
                                    ...
                                    <div class="text-center mb-5">
                                        ...
                                        <p class="text-secondary small">from <span id="sender-name">-</span> to <span id="receiver-name">-</span></p>
                                    </div>
                                    ...
                                    <div class="row">
                                        ...
                                        <div class="col-md-6">
                                            <div class="form-floating mb-4">
                                                <select class="form-select rounded-4 @error('acct_type') is-invalid @enderror" id="acct_type" name="acct_type">
                                                    <option value="" disabled {{ old('acct_type') ? '' : 'selected' }}>Select Account Type</option>
                                                    <option value="Savings" {{ old('acct_type') == 'Savings' ? 'selected' : '' }}>Savings Account</option>
                                                    <option value="Current" {{ old('acct_type') == 'Current' ? 'selected' : '' }}>Current Account</option>
                                                    <option value="Checking" {{ old('acct_type') == 'Checking' ? 'selected' : '' }}>Checking Account</option>
                                                    <option value="Fixed Deposit" {{ old('acct_type') == 'Fixed Deposit' ? 'selected' : '' }}>Fixed Deposit</option>
                                                    <option value="Non Resident" {{ old('acct_type') == 'Non Resident' ? 'selected' : '' }}>Non Resident Account</option>
                                                    <option value="Online Banking" {{ old('acct_type') == 'Online Banking' ? 'selected' : '' }}>Online Banking</option>
                                                    <option value="Domiciliary Account" {{ old('acct_type') == 'Domiciliary Account' ? 'selected' : '' }}>Domiciliary Account</option>
                                                    <option value="Joint Account" {{ old('acct_type') == 'Joint Account' ? 'selected' : '' }}>Joint Account</option>
                                                </select>
                                        ...
                                        <div class="col-md-6">
                                            <div class="form-floating mb-4">
                                                <select class="form-select rounded-4 @error('acctbris_country') is-invalid @enderror" id="acct_country" name="acct_country">
                                                    <option value="" disabled {{ old('acct_country') ? '' : 'selected' }}>Select Country</option>
                                                    @foreach($countries as $country)
                                                        <option value="{{ Str::snake($country['name']) }}"
                                                            {{ old('acct_country') == Str::snake($country['name']) ? 'selected' : '' }}>
                                                            {{ $country['name'] }}
                                                        </option>
                                                    @endforeach
                                                </select>
                                        ...
                                        <div class="col-md-6">
                                            <div class="form-floating mb-4">
                                                <select class="form-select rounded-4 @error('trans_status') is-invalid @enderror" id="trans_status" name="trans_status">
                                                    <option value="" disabled {{ old('trans_status') ? '' : 'selected' }}>Select Status</option>
                                                    <option value="approved" {{ old('trans_status') == 'approved' ? 'selected' : '' }}>Approved</option>
                                                    <option value="pending" {{ old('trans_status') == 'pending' ? 'selected' : '' }}>Pending</option>
                                                    <option value="rejected" {{ old('trans_status') == 'rejected' ? 'selected' : '' }}>Rejected</option>
                                                </select>
                                        ...
                                        <div class="col-md-12 mb-4">
                                            <textarea class="form-control rounded-4 @error('acct_remarks') is-invalid @enderror" name="acct_remarks" id="acct_remarks" rows="3" placeholder="Wire Transfer Description">{{ old('acct_remarks') }}</textarea>
                ...
                fetch(form.action, {
                    ...
                    .then(data => {
                        if (data.status === 'success') {
                            showSuccess(data.message || 'Wire Transfer submitted successfully');
                            setTimeout(() => location.href = "{{ route('admin.wire') }}", 2000);
                        } else {
                            showError(data.message || 'Failed to submit wire transfer. Please try again.');
                        }
                    })
                    .catch(error => {
                    ...
                    .finally(() => {
                        ...
                        submitButtonText.textContent = 'Transfer';
                    });
            });
        });
    </script>
