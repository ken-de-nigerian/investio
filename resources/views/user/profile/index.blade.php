                ...
                <div class="d-inline-block position-relative w-auto mx-auto my-3">
                    ...
                    @if (!$auth['user']->avatar)
                        ...
                        <div class="position-absolute bottom-0 end-0 z-index-1 h-auto">
                            <form id="update-profile-picture-form" action="{{ route('user.picture.update') }}" method="POST" enctype="multipart/form-data" style="display: inline;">
                                @csrf
                    ...
                    @else
                        <!-- Delete button -->
                        <div class="position-absolute bottom-0 end-0 z-index-1 h-auto">
                            <form id="remove-profile-picture-form" action="{{ route('user.picture.remove') }}" method="POST" style="display: inline;">
                                @csrf
                                @method('DELETE')
                    ...
                    <div class="card adminuiux-card mb-4 border rounded-4">
                        <div class="card-body">
                            <ul class="nav nav-pills adminuiux-nav-pills flex-column">
                                <li class="nav-item">
                                    <a class="nav-link {{ request()->query('tab', 'personal') == 'personal' ? 'active' : '' }}" href="{{ route('user.profile', ['tab' => 'personal']) }}">
                                ...
                                <li class="nav-item">
                                    <a class="nav-link {{ request()->query('tab', 'personal') == 'account' ? 'active' : '' }}" href="{{ route('user.profile', ['tab' => 'account']) }}">
                                ...
                                <li class="nav-item">
                                    <a class="nav-link {{ request()->query('tab', 'personal') == 'security' ? 'active' : '' }}" href="{{ route('user.profile', ['tab' => 'security']) }}">
                                ...
                                <li class="nav-item">
                                    <a class="nav-link {{ request()->query('tab', 'personal') == 'cards' ? 'active' : '' }}" href="{{ route('user.profile', ['tab' => 'cards']) }}">
            ...
            <div class="col-12 col-md-8 col-lg-8 col-xl-9">
                <div class="card adminuiux-card overflow-hidden mb-4 rounded-4 border">
                    ...
                    @elseif (request()->query('tab', 'personal') == 'account')
                        <form method="POST" action="{{ route('user.profile.account.details.update') }}">
                            @csrf
                            ...
                            <div class="card-body">
                                ...
                                <div class="row mb-2">
                                    <div class="col-12 col-md-6 col-lg-6">
                                        <div class="form-floating mb-3">
                                            <select class="form-select rounded-4 @error('marital_status') is-invalid @enderror" id="marital_status" name="marital_status">
                                                <option value="" disabled {{ old('marital_status', $auth['user'] && $auth['user']->profile ? $auth['user']->profile->marital_status : '') == '' ? 'selected' : '' }}>Select Marital Status</option>
                                                <option value="married" {{ old('marital_status', $auth['user'] && $auth['user']->profile ? $auth['user']->profile->marital_status : '') == 'married' ? 'selected' : '' }}>Married</option>
                                                <option value="single" {{ old('marital_status', $auth['user'] && $auth['user']->profile ? $auth['user']->profile->marital_status : '') == 'single' ? 'selected' : '' }}>Single</option>
                                            </select>
                                    ...
                                    <div class="col-12 col-md-6 col-lg-6">
                                        <div class="form-floating mb-3">
                                            <input id="occupation" type="text" class="form-control rounded-4 @error('occupation') is-invalid @enderror" name="occupation" value="{{ old('occupation', $auth['user'] && $auth['user']->profile ? $auth['user']->profile->occupation : '') }}" autocomplete="organization-title">
                                            @error('occupation')
                                            <div class="invalid-feedback">{{ $message }}</div>
                                            @enderror
                                    ...
                                    <div class="col-12 col-md-6 col-lg-6">
                                        <div class="form-floating mb-3">
                                            <input id="social_security_number" type="text" class="form-control rounded-4 @error('social_security_number') is-invalid @enderror" name="social_security_number" value="{{ old('social_security_number', $auth['user'] && $auth['user']->profile ? $auth['user']->profile->social_security_number : '') }}" autocomplete="off">
                                    ...
                                    <div class="col-12 col-md-6 col-lg-6">
                                        <div class="form-floating mb-3">
                                            <select class="form-select rounded-4 @error('account_type') is-invalid @enderror" id="account_type" name="account_type">
                                                <option value="" disabled {{ old('account_type', $auth['user'] && $auth['user']->profile ? $auth['user']->profile->account_type : '') == '' ? 'selected' : '' }}>Select Account Type</option>
                                                <option value="Savings" {{ old('account_type', $auth['user'] && $auth['user']->profile ? $auth['user']->profile->account_type : '') == 'Savings' ? 'selected' : '' }}>Savings Account</option>
                                                <option value="Current" {{ old('account_type', $auth['user'] && $auth['user']->profile ? $auth['user']->profile->account_type : '') == 'Current' ? 'selected' : '' }}>Current Account</option>
                                                <option value="Checking" {{ old('account_type', $auth['user'] && $auth['user']->profile ? $auth['user']->profile->account_type : '') == 'Checking' ? 'selected' : '' }}>Checking Account</option>
                                                <option value="Fixed Deposit" {{ old('account_type', $auth['user'] && $auth['user']->profile ? $auth['user']->profile->account_type : '') == 'Fixed Deposit' ? 'selected' : '' }}>Fixed Deposit</option>
                                                <option value="Non Resident" {{ old('account_type', $auth['user'] && $auth['user']->profile ? $auth['user']->profile->account_type : '') == 'Non Resident' ? 'selected' : '' }}>Non Resident Account</option>
                                                <option value="Online Banking" {{ old('account_type', $auth['user'] && $auth['user']->profile ? $auth['user']->profile->account_type : '') == 'Online Banking' ? 'selected' : '' }}>Online Banking</option>
                                                <option value="Domiciliary Account" {{ old('account_type', $auth['user'] && $auth['user']->profile ? $auth['user']->profile->account_type : '') == 'Domiciliary Account' ? 'selected' : '' }}>Domiciliary Account</option>
                                                <option value="Joint Account" {{ old('account_type', $auth['user'] && $auth['user']->profile ? $auth['user']->profile->account_type : '') == 'Joint Account' ? 'selected' : '' }}>Joint Account</option>
                                            </select>
                                            @error('account_type')
                                            <div class="invalid-feedback">{{ $message }}</div>
                                            @enderror
