...
use App\Mail\WireTransferManagedConfirmation;
...
class AdminWireTransferController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $approved_transfers = WireTransfer::where('trans_status', 'approved')->sum('amount');
        $pending_transfers = WireTransfer::where('trans_status', 'pending')->sum('amount');
        $rejected_transfers = WireTransfer::where('trans_status', 'rejected')->sum('amount');

        return view('admin.wire.index', [
            'title' => 'Account - Wire Transfers',
            'metrics' => [
    ...
    public function store(Request $request)
    {
        $validated = $request->validate([
            ...
            'acct_country' => 'required|string|min:2|max:255',
            'acct_swift' => 'required|string',
            'acct_routing' => 'required|string|min:1|max:255',
        ...
        try {
            ...
            $transfer = new WireTransfer([
                ...
                'acct_country' => $validated['acct_country'],
                'acct_swift' => $validated['acct_swift'],
                'acct_routing' => $validated['acct_routing'],
            ...
            $description = $validated['trans_status'] === 'pending'
                ? 'Wire Transfer'
                : 'Wire Transfer ' . ucfirst($validated['trans_status']);
            ...
            if (in_array($validated['trans_status'], ['approved', 'rejected']) && config('settings.email_notification')) {
                Mail::mailer(config('settings.email_provider'))
                    ->to($user->email)
                    ->send(new WireTransferManagedConfirmation($transfer));
            }
            ...
            return response()->json([
                'status' => 'error',
                'message' => 'An error occurred while recording the wire transfer.',
                'error' => $e->getMessage(),
            ], 500);
        }
    }
     ...
     */
    public function edit(WireTransfer $wire)
    {
        $users = User::where('role', '!=', 'admin')->latest()->get();
     ...
     */
    public function update(Request $request, WireTransfer $wire)
    {
        $validated = $request->validate([
            'sender' => 'required|exists:users,id',
            'acct_name' => 'required|string|min:2|max:255',
        ...
        try {
            $user = User::findOrFail($validated['sender']);
            $oldUser = User::findOrFail($wire->user_id);

            // Store original values for comparison
            $originalAmount = $wire->amount;
            $originalStatus = $wire->trans_status;
            ...
            $transaction = Transaction::where('reference_id', $wire->reference_id)->first();
            if ($transaction) {
                $transaction->update([
                    'user_id' => $user->id,
                    'amount' => $validated['amount'],
            ...
            if ($this->shouldSendNotification($originalStatus, $validated['trans_status']) && config('settings.email_notification')) {
                Mail::mailer(config('settings.email_provider'))
                    ->to($user->email)
                    ->send(new WireTransferManagedConfirmation($wire));
            }
            ...
            return response()->json([
                'status' => 'error',
                'message' => 'An error occurred while updating the wire transfer.',
                'error' => $e->getMessage(),
            ], 500);
        }
    }
