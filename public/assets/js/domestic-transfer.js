...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const domesticForm = document.getElementById('domestic-transfer');
    const storeDomesticTransferForm = document.getElementById('storeDomesticTransfer');
    const domTransferStoreBtn  = document.getElementById('domTransferStoreBtn');
    const fields = ['acct_name', 'account_number', 'bank_name', 'domestic_transfer_amount', 'acct_type', 'acct_remarks'];
    ...
    if (!domesticBtn || !domesticForm) return;
    ...
    domesticBtn.addEventListener('click', function (event) {
        event.preventDefault();

        domesticBtn.innerHTML = '<span class="spinner-border spinner-border-sm me-2" role="status"></span>Processing...';
        ...
        if (!isValid) {
            domesticBtn.innerHTML = 'Transfer';
            domesticBtn.disabled = false;
            return;
        }

        const formData = new FormData(domesticForm);

        fetch('/domestic/transfer/create', {
            method: "POST",
            body: formData,
            headers: {
            ...
            .then(data => {
                if (data.status === 'success') {
                    ...
                    const modalEl = document.getElementById('otpValidationModal');
                    if (modalEl) {
    ...
    // Submit & store domestic transfer
    domTransferStoreBtn.addEventListener("click", function (e) {
        e.preventDefault();

        const code = document.getElementById('dom_code').value.trim();
        ...
        domTransferStoreBtn.disabled = true;
        domTransferStoreBtn.innerHTML = `<span class="spinner-border spinner-border-sm me-2" role="status"></span>Sending...`;

        const formData = new FormData(storeDomesticTransferForm);

        fetch(storeDomesticTransferForm.action, {
            method: "POST",
            body: formData,
            headers: {
                "X-CSRF-TOKEN": document.querySelector('meta[name="csrf-token"]').content,
            ...
            .finally(() => {
                domTransferStoreBtn.disabled = false;
                domTransferStoreBtn.textContent = "Complete Transfer";
            });
    });
});
...
// Verify OTP
document.addEventListener('DOMContentLoaded', function () {
    const codeInput = document.getElementById('dom_code');
    const otpBoxes = document.querySelectorAll('#domOtpDisplay .otp-box');
    ...
    keypadKeys.forEach(key => {
        key.addEventListener('click', () => {
            ...
            if (key.id === 'domClearKey') {
                codeInput.value = '';
            } else if (key.id === 'domBackspaceKey') {
...
// Resend OTP
document.addEventListener('DOMContentLoaded', function () {
    const resendOtp = document.getElementById('domResendOtp');
